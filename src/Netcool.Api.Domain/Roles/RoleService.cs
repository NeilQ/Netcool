...
public sealed class RoleService : CrudService<Role, RoleDto, int, RoleRequest, RoleSaveInput>, IRoleService
{
    ...
    public override async Task BeforeCreate(Role entity)
    {
        ...
        var nameExist = Repository.GetQueryable().AsNoTracking().Any(t => t.Name == entity.Name);
        if (nameExist)
        {
    ...
    public override async Task BeforeUpdate(RoleSaveInput input, Role originEntity)
    {
        ...
        var nameExist = Repository.GetQueryable().AsNoTracking()
    ...
    public IList<PermissionDto> GetRolePermissions(int id)
    {
        ...
        var role = Repository.GetQueryable().AsNoTracking()
    ...
    public async Task SetRolePermissionsAsync(int id, IList<int> permissionIds)
    {
        ...
        var role = Repository.GetQueryable()
            .Include(t => t.RolePermissions)
        ...
        {
            var permissions = _permissionRepository.GetQueryable().AsNoTracking()
    ...
    protected override async void BeforeDelete(IEnumerable<int> ids)
    {
        ...
        var userRoles = _userRoleRepository.GetQueryable().AsNoTracking().Where(t => ids.Contains(t.RoleId)).ToList();
